...
const FLOW_NODES = [
  ...
  { label: "CLAUDE AI", desc: "claude-sonnet-4" },
  { label: "MCP BRIDGE", desc: "Tool protocol" },
  { label: "RELAYER", desc: "Partial fee payer" },
...
const AGENT_LOOP = [
  ...
  { n: "3", label: "tool_use: mint_tokens", desc: "Claude decides to call the mint tool via MCP" },
  { n: "4", label: "RELAYER /buy", desc: "MCP bridge calls POST /buy with wallet + amount" },
  ...
  { n: "7", label: "USER APPROVES", desc: "Your wallet popup: sign the buyer side" },
  { n: "8", label: "DONE ✓", desc: "250,000 $BAO credited to your wallet on-chain" },
];

const ENDPOINTS = [
  { method: "POST", path: "/buy", desc: "Mint tokens via bonding curve" },
  { method: "POST", path: "/sell", desc: "Sell tokens back to the curve" },
  { method: "GET", path: "/price", desc: "Fetch current price and curve state" },
  { method: "POST", path: "/user/register", desc: "Register wallet (sign once)" },
];

const MCP_TOOLS = [
  { name: "get_price", desc: "Returns current token price and bonding curve stats" },
  { name: "get_user_status", desc: "Returns mints used, remaining, and wallet bag size" },
  { name: "mint_tokens", desc: "Builds and returns a partial transaction for Claude to relay" },
    ...
    <main className="min-h-screen bg-[#F7C500] text-[#1A0500]">
      ...
      <section className="bg-[#C8102E] text-white py-16 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-5">
          <Link
            href="/"
            className="inline-block text-[11px] font-bold uppercase tracking-widest text-white/60 hover:text-white transition-colors mb-6"
          >
            ← Back to $BAO
          </Link>
      ...
      <section className="bg-[#FFF8E1] py-14">
        <div className="max-w-5xl mx-auto px-5">
          ...
          <p className="text-[#7A4200] text-sm mt-8 max-w-2xl leading-relaxed">
            ...
            approve. The relayer pays the gas so you only need to own SOL for the token purchase itself.
          </p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
            <div>
              ...
              <p className="text-[#7A4200] text-sm leading-relaxed mb-4">
                ...
                automatically credited to your registered wallet — no additional signatures needed
                on your part to <em>initiate</em> a mint (you still approve each final tx).
              </p>
              <p className="text-[#7A4200] text-sm leading-relaxed">
                The relayer covers the SOL for creating the PDA account. You pay nothing to
                register — only the 0.022 SOL per actual mint.
              </p>
            </div>
            ...
            <div className="bg-[#1A0500] p-5 font-mono text-sm">
              ...
              <div className="space-y-1.5 text-xs">
                ...
                <p className="text-[#FFF8E1] pl-4">payer: <span className="text-[#006064]">Relayer</span>,</p>
                <p className="text-[#FFF8E1] pl-4">system_program: SystemProgram,</p>
                <p className="text-white">{"}"}</p>
                <div className="h-px bg-white/10 my-3" />
                <p className="text-[#7A7070]">// UserAccount PDA seeds</p>
                <p className="text-[#D4AF37]">[b"user", wallet.key().as_ref()]</p>
                <div className="h-px bg-white/10 my-3" />
        ...
        <div className="max-w-5xl mx-auto px-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {AGENT_LOOP.map((step) => (
              <div
                key={step.n}
                className="bg-white border-2 border-[#C8102E] p-4 space-y-3"
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
            <div>
              <p className="text-[#7A4200] text-sm leading-relaxed mb-4">
                ...
                the relayer signs first (covering SOL rent and fees), then passes the transaction
                back to the user for their buyer signature.
              </p>
              <p className="text-[#7A4200] text-sm leading-relaxed mb-6">
                ...
                the registered user to also sign as buyer. Two signatures are mandatory; one is
                not enough.
              </p>
      ...
      <section className="bg-[#FFF8E1] py-14 border-t-2 border-[#C8102E]/20">
        <div className="max-w-5xl mx-auto px-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-8">
            {[
              { name: "BondingCurve PDA", seeds: '[b"bonding_curve"]', desc: "Stores virtual reserves, real SOL raised, and graduated flag" },
              { name: "UserAccount PDA", seeds: '[b"user", wallet]', desc: "Tracks mints used, mints remaining, and total SOL spent" },
              { name: "Token Mint", seeds: '[b"mint"]', desc: "The $BAO SPL token. Mint authority is the bonding curve PDA" },
            ].map((acct) => (
          ...
          <div className="bg-[#C8102E] p-6 text-center">
            <p className="text-[11px] uppercase tracking-widest text-white/60 mb-3">Pricing formula</p>
      ...
      <section className="bg-[#C8102E] text-white py-14">
        <div className="max-w-5xl mx-auto px-5">
          ...
          <p className="text-white/80 text-sm leading-relaxed max-w-2xl mb-4">
            Claude uses MCP tools natively through the Anthropic API. There is no browser
            extension, no plugin, and no third-party wallet. The MCP server runs alongside the
            relayer and exposes typed tools that Claude can call during a conversation.
          </p>
          <p className="text-white/80 text-sm leading-relaxed max-w-2xl mb-10">
            When Claude decides to mint, it emits a <span className="font-mono font-bold bg-white/10 px-1">tool_use</span> block
            in its response. The MCP bridge intercepts this, calls the relayer, and returns the
            transaction as a <span className="font-mono font-bold bg-white/10 px-1">tool_result</span>. Claude then surfaces
            it to you to approve in your wallet.
          </p>
